...
function OctordleScoreByDate() {
    ...
    const fetchDataByDate = (date) => {
        ...
        .then((response) => {
            if (response.data.status === "success") {
                setStatsChart(response.data.octordlescore);
                setDataFetched(true);
    ...
    const ExampleCustomInput = forwardRef(({ value, onClick }, ref) => {
        return (
            <>
                <Button className={`example-custom-input px-5 btn btn-primary octordle-btn`} onClick={onClick} ref={ref}>
            Go To Date
        </Button>
                ...
                <>
                    ...
                    {statsChart.length > 0 ? (
                        statsChart.map((item, index) => {
                            ...
                            const octordleScore = extractBoardLines(item.octordlescore);
...
export default OctordleScoreByDate;
